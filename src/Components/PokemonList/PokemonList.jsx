    ...
    return (
        <div className="page">
            <div className="pokemon_wrapper">
                {(stateList.isLoading) ? 'Loading...' :
                    stateList.pokemonList.map((p) => <Pokemons name={p.name} image={p.image} key={p.id} id={p.id} />
                    )
                }
            </div>
            <div className="controls">
                <button disabled={stateList.PrevUrl == null}
                    onClick={() => {
                        const urlToSet = stateList.PrevUrl;
                        setstateList({ ...stateList, PokedexURL: urlToSet })
                    }}
                >Prev</button>
                <button disabled={stateList.nextUrl == null}
